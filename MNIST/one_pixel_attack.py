...
def one_pixel_attack(model, img, true, d=1, pop=50, iters=100):
    ...
    for gen in range(iters):
        ...
        # Batch predict
        probs = model.predict(adv_batch, verbose=0)  # shape (pop, num_classes)
...
if __name__=='__main__':
    ...
    mn = MNIST()
    X = mn.test.data['images']
    y = mn.test.data['labels']
    n = 6
    x0 = np.expand_dims(X[n], -1)
    true_label = y[n]
    pred_before = np.argmax(model.predict(np.expand_dims(x0, 0), verbose=0))
    d = 3
    orig, adv, orig_lbl, adv_lbl = one_pixel_attack(model, x0, true_label, d, pop=1000, iters=200)
    if adv is not None:
        pred_after = np.argmax(model.predict(np.expand_dims(adv, 0), verbose=0))
        ...
        axs[0].set_title(f'Original (Pred: {pred_before})')
        axs[0].axis('off')

        axs[1].imshow(adv.squeeze(), cmap='gray')
        axs[1].set_title(f'{d} pixels changed (Pred: {pred_after})')
        axs[1].axis('off')
